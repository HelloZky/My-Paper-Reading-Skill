...
| **产品环境**(马车) | Obsidian 库、`paper.py`、NotebookLM CLI、`notes-web/`、Marp | 探测后按需使用,缺了就降级 |
...
5. **生成 Marp**(Step 5)。
6. **Markdown QA 门禁**(Step 6,**硬门禁**)— 不过不准勾 `markdown_done`。
...
1. **不编造**:实验数字、模块名、公式推导、SOTA 结论一律不许猜。缺失只能用固定文案:`论文未报告` / `无法从文中确认` / `未提供公开链接` / `未执行外部检索,无法确认`。
...
| 能力 | 依赖 | 不可用时 |
|---|---|---|
| 视觉材料(slides/infographic) | **方法A**:`notebooklm` CLI(不依赖 paper.py);**方法B 回退**:父目录 `paper.py` + Python 环境 | 两者都不可用才标 `blocked`,仅出 Markdown |
...
5. 内置兜底:`output_root`→cwd(**不静默**,写进 plan.md 与汇报);`python`→`python3`;`notebooklm_cli`→`notebooklm`;`paper_py`→无(视觉方法B blocked)
