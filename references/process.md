...
## Step 10 — 概念卡片推荐(收尾,默认执行,只推荐不擅自建卡)

> 目的:让知识随阅读复利——每处理完一篇,主动向用户**推荐**值得沉淀成概念卡片的**真·独立通用概念**,但**不擅自建卡**(建卡=改用户知识层,交用户拍板;与"不擅自改已有笔记"同源)。

在 `markdown_done` 后(NotebookLM/视觉是否完成都不影响本步),做四件事:

1. **抽候选**:从 6 份笔记(尤其 `方法介绍`/`实验结果`)里挑出被反复用到、或对理解本文关键的概念/方法/指标/统计工具。
2. **过质量闸——只留"真·独立通用概念"**(这是本步的核心,宁缺毋滥):
   - **真·独立**:自足的概念,不是"本文方法名"的马甲。`认知反思测验 CRT`✓;`本文的 CUV 签名模块`✗。
   - **通用可复用**:能跨多篇论文/多个领域出现或被引用。`辛普森悖论`、`贝叶斯多层建模`、`泛化信任`、`对比学习`✓;`本文德语音频素材偏差`、`该系统的某超参组合`✗。
   - **原子**:一个概念一张,别做打包卡(见用户记忆 `[[atomic-concept-cards]]`)。
   - 一票否决:论文特有术语、一次性实验设置、只在本文成立的组合、纯品牌/系统名——都不推荐。
3. **去重**:对每个候选,查 `output_root` 同级的 `概念卡片/`(或用户对应知识层目录)是否已存在同名卡:
   - **已存在** → 不重复建,提示"可在该卡'相关论文'节补一条反向链回本文";
   - **不存在** → 列为"建议新建"。
4. **汇报(不建卡)**:在最终交付信息里给一个简短清单,每条 = `概念名` + 一句"为什么值得独立成卡/如何跨论文复用",并标注"已存在/建议新建"。**建卡与否等用户确认**;用户同意后再用 `0-模版` 的概念卡片模板起草,并与本文双向 `[[链接]]`。

> 与 note-rules 配合:笔记正文**只对已有卡的概念**加了 `[[链接]]`(不制造死链);Step 10 独立扫描笔记内容,把**尚无卡**的真·独立通用概念收敛成推荐清单。**只推荐、不落地**,选择权留给用户;用户建卡后,可回笔记把该概念补成 `[[链接]]`。
