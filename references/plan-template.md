...
- **`visuals_done` 是原子套件**:slide-deck 与 infographic **两者都下载落盘并校验通过**才置 `done`;只完成一个 → 用 `running`(别置 done)。
- **NotebookLM/视觉跳过用 `skipped` 并在失败日志写原因**:验证器只读本状态表的精确任务名(`notebooklm_reports_*` / `visuals_*`)判 blocked/skipped,失败日志的自由文本不影响判定——但跳过仍要如实记。
