...
- **概念前向链接(与知识层复利)**:笔记正文出现**真·独立通用概念**(能跨论文复用的原子概念,如 `[[认知反思测验 CRT]]`、`[[辛普森悖论]]`、`[[贝叶斯多层建模]]`、`[[对比学习]]`)时,用 `[[概念名]]` 前向链接它;概念卡片不存在也照链(未解析链接="这张卡值得建"的标记,交给 Step 10 汇总推荐)。**只链真·独立通用概念**,别把"本文方法名/本文特有术语/一次性组合"当概念链(那是论文内容,不是可复用概念)。链接目标用概念卡片的完整文件名主名,可加 `|别名` 显示短名(如 `[[泛化信任 Generalized Trust|泛化信任]]`)。这与 Step 10 的推荐、以及概念卡片"相关论文"节的反向链接共同构成双链闭环。
