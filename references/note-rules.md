...
- **概念链接(只链已有卡,不制造死链)**:写笔记前**先列一遍 `概念卡片/`(`output_root` 同级)已有哪些卡**;正文出现某概念时,**仅当该概念已有对应卡片**才用 `[[卡名|别名]]` 链接它(目标用卡的完整文件名主名,可加 `|别名` 显示短名,如 `[[泛化信任 Generalized Trust|泛化信任]]`)。**没有卡就写纯文本——绝不为不存在的卡制造未解析链接**(用户明确不要满屏死链)。要不要为某概念新建卡,交给 Step 10 推荐、用户拍板;**卡建好后**才回笔记把对应概念补成 `[[链接]]`。判断哪些概念该链时,仍只认**真·独立通用概念**,别把"本文方法名/特有术语/一次性组合"当概念(那是论文内容)。已有卡的链接 + 概念卡片"相关论文"节的反向链接,构成双链闭环。
